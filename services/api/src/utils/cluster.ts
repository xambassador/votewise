...
const grace = APP_SETTINGS.WORKER_GRACE_TIMEOUT;
const signals: NodeJS.Signals[] = ["SIGINT", "SIGTERM", "SIGQUIT"];

type Configs = {
  master: (pid: number) => void | Promise<void>;
  worker: (pid: number, disconnect: () => void) => void | Promise<void>;
  count?: number;
};
...
export async function taskforce(config: Configs) {
  ...
  const shutdown = (signal: NodeJS.Signals) => {
    return () => {
      ...
      setTimeout(() => {
        logger.info(`Grace period ended. Forcing shutdown if needed`);

        // Check if there are any workers left
        if (Object.values(cluster.workers || {}).length === 0) {
          logger.info(`No workers left. Gracefully Exiting...`);
          process.exit();
        }
  ...
  if (cluster.isPrimary) {
    ...
    cluster.on("disconnect", (w) => {
      logger.info(`The worker #${w.id} has disconnected`);
      if (!running) return;
      cluster.fork();
    });
