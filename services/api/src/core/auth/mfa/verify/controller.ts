...
export class Controller {
  ...
  async handle(req: Request, res: Response) {
    ...
    this.ctx.assert.resourceNotFound(!_factor, "Factor not found");
    this.ctx.assert.resourceNotFound(_factor?.user_id !== payload.sub, "Factor not found");

    const _challenge = await this.ctx.challengeRepository.findById(body.challenge_id);
    this.ctx.assert.resourceNotFound(!_challenge, "MFA factor with the provided challenge ID not found");
    ...
    this.ctx.assert.unprocessableEntity(
      isValidChallenge,
      "Challenge and verify IP address mismatch. Try enrollment again"
    );

    const fiveMinutesAgoInSecs = 300;
    this.ctx.assert.unprocessableEntity(
      this.hasExpired(challenge.created_at, fiveMinutesAgoInSecs),
      `MFA challenge ${challenge.id} has expired, verify against another challenge or create a new challenge.`
    );
    ...
    this.ctx.assert.unprocessableEntity(!valid, "Invalid TOTP code provided");
