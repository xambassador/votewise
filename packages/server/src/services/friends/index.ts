...
class FriendsService {
  ...
  async addFriend(userId: number, friendId: number) {
    ...
    if (isFriend && isFriend.type === "FRIENDS") {
      throw new ServerError(StatusCodes.BAD_REQUEST, ALREADY_FRIENDS_MSG);
    }

    if (isFriend && isFriend.type === "PENDING_USER_TO_FRIEND_REQUEST") {
      throw new ServerError(StatusCodes.BAD_REQUEST, "Friend request already sent");
    }
  ...
  async acceptOrRejectFriendRequest(
    ...
    if (type === "ACCEPT") {
      const data = await this.acceptFriendRequest(userId, friendId, requestId);
      return data;
    }
    const data = await this.rejectFriendRequest(userId, friendId, requestId);
    return data;
  }
  ...
  async acceptFriendRequest(userId: number, friendId: number, requestId: number) {
    ...
    if (!request) {
      throw new ServerError(StatusCodes.NOT_FOUND, FRIEND_REQUEST_NOT_FOUND_MSG);
    }
    ...
    if (request.friend_id !== userId) {
      throw new ServerError(StatusCodes.UNAUTHORIZED, UNAUTHORIZED_MSG);
    }
