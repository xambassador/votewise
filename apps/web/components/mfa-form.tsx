...
export function MFAForm(props: React.PropsWithChildren<Props>) {
  const { qrCode, secret, children, onVerify: _onVerify, submitButtonProps, isLoading = false } = props;
  const value = useRef("");
        ...
        <div className="flex flex-col gap-5">
          <div className="flex flex-col gap-3">
            ...
            <div className="rounded-[1.25rem] bg-nobelBlack-200 py-6 grid place-items-center min-h-[10.625rem] max-h-[10.625rem]">
              {isLoading ? <div className="text-gray-500">Loading QR Code...</div> : <QRCode url={qrCode} />}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-3">
            ...
            <InputField>
              {isLoading ? (
                <span className="text-gray-500">Loading secret...</span>
              ) : (
                <>
