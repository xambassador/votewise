...
import { Avatar, Button, InputField, TextAreaField, ToggleField } from "@votewise/ui";
...
function Toggle() {
  ...
  return (
    <ToggleField
      ...
      checked={enabled}
      onChange={setEnabled}
      labelProps={{ className: "text-sm text-gray-600" }}
    />
  );
}
  ...
  return (
    <div className="shadow-modal flex w-[540px] shrink-0 flex-col items-start justify-center gap-5 rounded-lg bg-white p-9">
      ...
      <div className="relative flex shrink-0 flex-row items-start justify-start gap-4 self-stretch">
        ...
        <Button className="p-2.5 text-gray-600" secondary onClick={onClose}>
          Cancel
        </Button>
      </div>
    </div>
